...
try {
    ...
    // 3. Marca a reserva como excluída (excluido = 1)
    $sql_delete_reserva = "UPDATE reservas SET excluido = 1 WHERE id_reserva = ?";
    $stmt_delete_reserva = $conexao->prepare($sql_delete_reserva);
    $stmt_delete_reserva->bind_param("i", $idreserva);
    ...
    // 5. Se todas as operações foram bem-sucedidas, efetiva as mudanças no banco
    mysqli_commit($conexao);
    $_SESSION['message'] = [
        'type' => 'success',
        'text' => 'Agendamento excluído e quarto liberado com sucesso!'
    ];
